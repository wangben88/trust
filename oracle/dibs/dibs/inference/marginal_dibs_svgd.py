...
from oracle.dibs.dibs.inference.dibs import DiBS

## LIST OF MODIFICATIONS
# 1) in sample_particles, after each svgd_step

## NOTES:
# Not possible, I think, to avoid recompiling each time we are sampling from a different node.
# This is because we are indexing the graph array with variable indices which is not a jittable
# operation. As such, we must incur the jit compilation cost for every new node.
# If we pass the same MarginalDiBS object each time we want to sample a node, we get issues
# as jit compiles the MarginalDiBS object statically the first time, and changing the .s1s2
# property does not change this.
...
class MarginalDiBS(DiBS):
    ...
    #@functools.partial(jit, static_argnums=(0,), static_argnames=("n_steps", "callback", "callback_every"))
    def sample_particles(self, *, init_particles_z, key, callback=None, callback_every=0, budget=None, n_steps=None):
        """
        ...
        if budget is not None:
            ...
            for t in it:
                ...
                if callback and callback_every and (((t+1) % callback_every == 0) or (t == (n_steps - 1))):
                    ...
                    # zero out the appropriate
                    # if self.s1s2 is not None:
                    #     u, v = z[..., 0], z[..., 1]
                    #     s1, s2 = self.s1s2
                    #     u_zerod = jnp.zeros_like(u)
                    #     u_zerod[..., jnp.concatenate([s1, s2]), :] = u[..., jnp.concatenate([s1, s2]), :]
                    #     v_zerod = jnp.zeros_like(v)
                    #     v_zerod[..., s2, :] = v[..., s2, :]
                    #
                    #     u, v = u_zerod, v_zerod
