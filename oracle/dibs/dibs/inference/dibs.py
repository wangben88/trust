...
from oracle.dibs.dibs.utils.graph import acyclic_constr_nograd
from oracle.dibs.dibs.utils.func import expand_by

## LIST OF MODIFICATIONS
# 1) Added s1s2 argument to __init__, which is used in edge_probs, edge_log_probs to set non s1s2 columns to 0
#    cf. Notes in book, this should be enough to modify DiBS for seed method.
#    similar change for particle_to_g_lim
#    also now added similar change to particle_to_soft_graph
...
class DiBS:
    ...
    Args:
        ...
        s1s2: pair of sets of variables (s1, s2)
    """
    ...
    def restrict_graph_s1s2(self, from_arr, to_arr):
        """
        Fills values of from_arr into array to_arr, restricted to the columns of from_arr in s2 and rows of from_arr
        in s1 union s2. Returns result in new array.

        Args:
            from_arr: matrix of shape [..., d, d]
            to_arr: matrix of shape [..., d, d]

        Returns:
            matrix of shape [..., d, d]
        """
        #
        if self.s1s2 is not None:
            s1, s2 = self.s1s2[0], self.s1s2[1]
            new_arr = to_arr.at[..., :, s2].set(
                to_arr.at[..., :, s2].get().at[..., jnp.concatenate([s1, s2]), :].set(
                from_arr[..., :, s2][..., jnp.concatenate([s1, s2]), :])
            )
        else:
            raise Exception
        return new_arr
    ...
    def particle_to_g_lim(self, z):
        ...
        if self.s1s2 is not None:
            g_zerod = jnp.zeros_like(g_samples)
            g_samples = self.restrict_graph_s1s2(g_samples, g_zerod)
    ...
    def particle_to_soft_graph(self, z, eps, t):
        ...
        if self.s1s2 is not None:
            soft_zerod = jnp.zeros_like(soft_graph)
            soft_graph = self.restrict_graph_s1s2(soft_graph, soft_zerod)
    ...
    def particle_to_hard_graph(self, z, eps, t):
        ...
        if self.s1s2 is not None:
            hard_zerod = jnp.zeros_like(hard_graph)
            hard_graph = self.restrict_graph_s1s2(hard_graph, hard_zerod)
    ...
    def edge_probs(self, z, t):
        ...
        if self.s1s2 is not None:
            probs_zerod = jnp.zeros_like(probs)
            probs = self.restrict_graph_s1s2(probs, probs_zerod)
    ...
    def edge_log_probs(self, z, t):
        ...
        #set appropriate log probs to -inf, disables gradients for these entreis of G.


        if self.s1s2 is not None:
            log_probs_pos_zerod, log_probs_neg_zerod = jnp.full_like(log_probs, -jnp.inf), jnp.full_like(log_probs_neg, 0.0)

            log_probs = self.restrict_graph_s1s2(log_probs, log_probs_pos_zerod)
            log_probs_neg = self.restrict_graph_s1s2(log_probs_neg, log_probs_neg_zerod)
    ...
    def grad_theta_likelihood(self, single_z, single_theta, t, subk):
        ...
        stable_grad = tree_map(
            lambda sign_leaf_theta, log_leaf_theta:
            (sign_leaf_theta * jnp.exp(
                log_leaf_theta - jnp.log(n_mc_numerator) - log_denominator + jnp.log(n_mc_denominator))),
            sign, log_numerator)
